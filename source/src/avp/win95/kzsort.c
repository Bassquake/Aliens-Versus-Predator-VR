	...
	DrawingAReflection=0;
	{
		...
		else
		{
			...
			if (vr_is_rendering)
			{
				 ...
				 * BSP: GL_LEQUAL + depth write (standard).
				 * Entities: GL_LEQUAL + no depth write so coplanar entity faces (e.g. TV bezel)
				 * both pass against the BSP depth from this and farther modules — not against
				 * each other's written depth. Entities only test against BSP from modules that
				 * have already been rendered (far-to-near order), avoiding false occlusion by
				 * nearer-module BSP geometry displaced by the VR IPD parallax offset. */
				int m;
				for (m = numVisMods - 1; m >= 0; m--)
				{
					MODULE *modulePtr = SortedModules[m].DispPtr->ObMyModule;

					CheckWireFrameMode(WireFrameMode & 1);
					AddShape(SortedModules[m].DispPtr, VDBPtr);
					#if MIRRORING_ON
					if (MirroringActive)
					{
						DISPLAYBLOCK *dptr = SortedModules[m].DispPtr;
						ReflectObject(dptr);
						MakeVector(&dptr->ObWorld, &VDBPtr->VDB_World, &dptr->ObView);
						RotateVector(&dptr->ObView, &VDBPtr->VDB_Mat);
						DrawingAReflection = 1;
					...
					VR_DepthObjects();
					{
						...
						{
							o--;
							if (VisibleObjects[o].SortKey == m && !VisibleObjects[o].DrawBeforeEnvironment)
							{
								DISPLAYBLOCK *dptr = VisibleObjects[o].DispPtr;
								AddShape(dptr, VDBPtr);
								#if MIRRORING_ON
