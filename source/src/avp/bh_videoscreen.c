	...
	if(dptr)
  	{
		...
		if (lightPtr)
		{
			...
			{
				/* Self-illumination: short range so it only covers the screen surface */
				lightPtr->LightBright = ONE_FIXED;
				lightPtr->LightFlags  = LFlag_Omni;
				lightPtr->LightType   = LightType_PerVertex;
				lightPtr->LightRange  = 500;
				lightPtr->RedScale    = ONE_FIXED;
				lightPtr->GreenScale  = ONE_FIXED;
				lightPtr->BlueScale   = ONE_FIXED;

				/* Add a second light for FMV-coloured environment cast */
				LIGHTBLOCK *fmvLight = AllocateLightBlock();
				if(fmvLight)
				{
					fmvLight->LightBright = ONE_FIXED;
					fmvLight->LightFlags  = LFlag_Omni;
					fmvLight->LightType   = LightType_PerVertex;
					fmvLight->LightRange  = 5000;
					fmvLight->RedScale    = FmvColourRed;
					fmvLight->GreenScale  = FmvColourGreen;
					fmvLight->BlueScale   = FmvColourBlue;
					AddLightBlock(dptr, fmvLight);
				}
			}
			else if(sbPtr->SBdptr->ObNumLights >= 2)
			{
				/* Update FMV colour cast light every frame */
				LIGHTBLOCK *fmvLight = dptr->ObLights[1];
				if(fmvLight)
				{
					fmvLight->RedScale   = FmvColourRed;
					fmvLight->GreenScale = FmvColourGreen;
