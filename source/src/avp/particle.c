...
/* Render pass only — no physics update. Re-marks all particles ready and runs
...
#endif
{
	...
	/* Render wall decals (bullet holes, scorch marks) for this eye.
	   Physics/size updates were already done by HandleDecalSystem on eye 0. */
	RenderDecalsOnly();
	D3D_DecalSystem_Setup();
	OutputTranslucentPolyList();

	/* Render volumetric explosions (grenade launcher etc.) for this eye.
	   Physics/position updates run only in HandleParticleSystem (eye 0);
	   RenderExplosionSurface is pure draw-only so safe to call per-eye. */
	{
		int exp_i;
		for (exp_i = 0; exp_i < MAX_NO_OF_EXPLOSIONS; exp_i++)
		{
			if (ExplosionStorage[exp_i].LifeTime)
...
static void HandleVolumetricExplosion(VOLUMETRIC_EXPLOSION *expPtr)
{
	...
	/* Rendering is now done per-eye in RenderParticlesOnly → RenderExplosionSurface. */
