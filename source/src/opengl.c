...
static const char *game_fs =
		"#version 100\n"
		"precision mediump float;\n"
		"varying vec2      vUV;\n"
		"varying vec4      vColor;\n"
		"uniform sampler2D uTex;\n"
		"uniform int       uSpecularPass;\n"  // 0=normal, 1=specular second pass
		"uniform int       uNoTexture;\n"     // 1 when no texture bound
		"void main() {\n"
		"    vec4 texel = (uNoTexture == 0) ? texture2D(uTex, vUV) : vec4(1.0);\n"
		"    vec4 color;\n"
		"    if (uSpecularPass == 0) {\n"
		"        color = texel * vColor;\n"   // GL_MODULATE
		"    } else {\n"
		// specular second pass: replace RGB with vertex colour, keep texture alpha
		"        color = vec4(vColor.rgb, texel.a * vColor.a);\n"
		"    }\n"
...
static GLuint compile_game_shader(GLenum type, const char *src)
{
	...
	if (!ok) {
		...
		fprintf(stderr, "game shader compile error: %s\n", log);
	}
	return s;
}
...
{
    GLuint vs = compile_game_shader(GL_VERTEX_SHADER,   game_vs);
    GLuint fs = compile_game_shader(GL_FRAGMENT_SHADER, game_fs);
    g_prog = glCreateProgram();
    glAttachShader(g_prog, vs);
    glAttachShader(g_prog, fs);
    glBindAttribLocation(g_prog, 0, "aPos");
    glBindAttribLocation(g_prog, 1, "aUV");
    glBindAttribLocation(g_prog, 2, "aColor");
    ...
    if (!linked) {
        ...
        fprintf(stderr, "game shader link error: %s\n", log);
    }
    glDeleteShader(vs);
    glDeleteShader(fs);
...
/* VR HUD clip-space controls — active only during MaintainHUD() in VR.
   ...
   vr_hud_offset_x/y: shift the entire HUD in clip space after scaling.
                      Range is roughly [-1, 1]; positive X = right, positive Y = up. */
float vr_hud_clip_scale = 1.0f;
...
void RestoreGameShaderState(void)
{
	...
	CurrentFilteringMode = FILTERING_BILINEAR_ON;
...
/* Flush any batched triangles to GL without touching the depth buffer.
   Call before changing depth func, depth mask, or color mask so that
   the batched triangles render under the state they were queued with. */
...
void VR_RestoreGLState(void)
{
	extern void InitOpenGL(void);
	InitOpenGL();  /* re-enables attribs, sets program, resets blend */
}

void VR_DrawTestTriangle(void)
{
	glUseProgram(g_prog);
	...
	glEnableVertexAttribArray(g_aPos);
	glVertexAttribPointer(g_aPos, 4, GL_FLOAT, GL_FALSE, 0, tri);
	glEnableVertexAttribArray(g_aColor);
	glVertexAttribPointer(g_aColor, 4, GL_UNSIGNED_BYTE, GL_TRUE, 0, col);
	glDisableVertexAttribArray(g_aUV);
	glUniform1i(g_uNoTex, 1);
	glUniform1i(g_uSpecularPass, 0);
	
	glDrawArrays(GL_TRIANGLES, 0, 3);
	
	GLenum err = glGetError();
	if (err) SDL_Log("VR_DrawTestTriangle GL err: 0x%x", err);
}
void VR_InvalidateTextureCache(void)
{
    //SDL_Log("VR_InvalidateTextureCache: pglBindTexture=%p", (void*)pglBindTexture);
    CurrentlyBoundTexture = NULL;
    pglBindTexture(GL_TEXTURE_2D, 0);
}