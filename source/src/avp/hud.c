...
extern int vr_eye_index; /* defined in avpview.c; 0=first eye, 1=second */
...
//	GlobalAmbience=16384;
	...
	else if (playerStatusPtr->IsAlive)
	{
		...
		switch(AvP.PlayerType)
		{
			case I_Marine:
			{
				/* In VR the 3D weapon model is rendered per-eye by KRenderItems;
				...
				DisplayMarinesAmmo();
		  		if (ShowCrosshair) DrawMarineSights();
	       	...
	       	case I_Predator:
			{
				if (!VR_IsIn3DMode()) HandlePredatorWeapon();
				CheckWireFrameMode(0);
