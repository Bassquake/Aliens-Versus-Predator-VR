...
/* Render-only pass — skips growth-rate physics so it can be called a second
   time per frame (e.g. VR right eye) without double-advancing decal sizes. */
void RenderDecalsOnly(void)
{
	D3D_DecalSystem_Setup();
	...
	/* Predator laser-sight halos — mirrors the ActiveBlockList loop in
	   HandleDecalSystem so the right eye also gets these RenderLightFlare calls. */
	{
		extern int NumActiveBlocks;
		extern DISPLAYBLOCK *ActiveBlockList[];
		extern void RenderLightFlare(VECTORCH *positionPtr, unsigned int colour);
				...
				switch (sbPtr->I_SBtype)
				{
					case (I_BehaviourNetGhost):
					{
						...
						if (ghostData->type == I_BehaviourPredatorPlayer)
						{
							extern DPID AVPDPNetID;
							int playerIndex = PlayerIdInPlayerList(ghostData->playerId);
							if (playerIndex == NET_IDNOTINPLAYERLIST) continue;
							if ((ghostData->CurrentWeapon != WEAPON_PRED_RIFLE)
							  &&(ghostData->CurrentWeapon != WEAPON_PRED_SHOULDERCANNON)) continue;
							if (AVPDPNetID == PredatorLaserSights[playerIndex].TargetID)
							{
								SECTION_DATA *plasma_muzzle = GetThisSectionData(
									ghostData->HModelController.section_data, "dum flash");
								if (plasma_muzzle != NULL)
									RenderLightFlare(&plasma_muzzle->World_Offset, 0xffff0000);
							}
							else
