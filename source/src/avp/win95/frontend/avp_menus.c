...
{
	SDL_Log("BOOT: AvP_MainMenus returned");
	#if 0
	...
	#endif
    SDL_Log("MENU: before LoadAllAvPMenuGfx");
	LoadAllAvPMenuGfx();
	...
	ResetFrameCounter();
 	...
 	if (!LobbiedGame)	// Edmond
        SDL_Log("MENU: before PlayIntroSequence");
		PlayIntroSequence();
        SDL_Log("MENU: before while loop");
	...
	{
		/* Load display settings from the first real saved profile before rendering.
		...
		ExamineSavedUserProfiles();
		{
			...
			int savedMarineLeftArmVisible = MarineLeftArmVisible;
			if (NumberOfUserProfiles() > 0)
			{
				GetFirstUserProfile(); /* skip "New Profile" placeholder at index 0 */
				UserProfilePtr = GetNextUserProfile(); /* first real saved profile */
				if (UserProfilePtr)
				{
					...
					savedMarineLeftArmVisible = MarineLeftArmVisible;
				}
			}
			...
			MarineLeftArmVisible = savedMarineLeftArmVisible;
		}
   ...
   //	BezierCurve();
		...
		ShowMenuFrameRate();
		FlipBuffers();
        //SDL_Log("DEBUG REACHED HERE INSIDE AvPMainMenus() in avp_menus.c*******");
		FrameCounterHandler();
		PlayMenuMusic();
	...
	int result = (AvPMenus.MenusState == MENUSSTATE_STARTGAME);
	
	SDL_Log("AvP_MainMenus: returning %d", result);
	return result;
